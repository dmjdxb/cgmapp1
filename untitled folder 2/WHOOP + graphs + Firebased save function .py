    ...
    if app_user_id:
        ...
        if doc.exists:
            try:
                ...
                # WHOOP Developer API endpoints
                recovery_resp = requests.get("https://api.prod.whoop.com/developer/v1/recovery", headers=headers)
                strain_resp   = requests.get("https://api.prod.whoop.com/developer/v1/cycle", headers=headers)
                sleep_resp    = requests.get("https://api.prod.whoop.com/developer/v1/activity/sleep", headers=headers)

                st.write("Recovery status:", recovery_resp.status_code)
                st.write("Strain status:", strain_resp.status_code)
                st.write("Sleep status:", sleep_resp.status_code)
                ...
                if (
                    ...
                    try:
                        # ✅ Parse WHOOP metrics
                        strain_score = latest_strain.get("score", {}).get("strain", 12)
                        strain = round(float(strain_score))

                        recovery_score = latest_recovery.get("score", {}).get("recovery_score", 65)
                        recovery = round(float(recovery_score))

                        sleep_millis = latest_sleep.get("score", {}).get("stage_summary", {}).get("total_in_bed_time_milli", 27000000)
                        sleep_hours = round(float(sleep_millis) / (1000 * 60 * 60), 1)

                        # ✅ Display actual live metrics
                        ...
                        st.session_state.db.collection("users").document(app_user_id).collection("whoop_logs").add(log_data)
                        st.success("🗂️ WHOOP data logged for today.")

                        # ✅ WHOOP Trend Chart (last 7 logs)
                        st.subheader("📈 WHOOP Trends (Last 7 Days)")

                        try:
                            logs_ref = st.session_state.db.collection("users").document(app_user_id).collection("whoop_logs").order_by("timestamp", direction=firestore.Query.DESCENDING).limit(7)
                            ...
                            for log in logs:
                                ...
                                if timestamp:
                                    data.append({
                                        ...
                                        "Sleep Hours": entry.get("sleep_hours")
                                    })
                            ...
                            if data:
                                ...
                                st.dataframe(df)
                                st.line_chart(df.set_index("Date")[["Strain", "Recovery", "Sleep Hours"]])
                            else:
                                st.info("No WHOOP logs available yet. Log some data first.")
                        except Exception as e:
                            st.error(f"Failed to fetch WHOOP trend data: {e}")

                    except Exception as e:
                        st.warning(f"WHOOP parsing error: {e}")
