...
openai.api_key = "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"
...
# ========== PAGE 2: ChatGPT Meal Plan ==========
    ...
    st.title("🥗 Generate a Sample Meal Plan Using ChatGPT")

    with st.form("meal_form_chatgpt"):
        generate_chatgpt = st.form_submit_button("Generate Meals")
    ...
    if generate_chatgpt:
        ...
        prompt = f'''
        I need a daily meal plan for a {diet_choice} diet with the following macros:
        Protein: {protein_g}g
        Carbs: {carbs_g}g
        Fat: {fat_g}g
        ...
        try:
            ...
            meals = response.choices[0].text.strip().split("\n")
            for idx, meal in enumerate(meals):
                if meal.strip():
                    st.write(f"**Meal {idx+1}:** {meal}")
